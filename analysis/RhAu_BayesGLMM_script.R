...
# Nest success data
nest_data <- read.csv(here("data","RhAu2.csv"), fileEncoding="UTF-8-BOM")
...
# Monthly 1854-2019
pdo <- read.csv(here("data","pdo.timeseries.ersstv3b.csv"), na.strings = "-9999.000")
colnames(pdo)[2] <- "pdo"
...
mei <- read.csv(here("data","MEIv2.csv"))
...
# Monthly 2009-2018
sst_DI <- read.csv(here("data","SST_DI_2002_2019.csv"), skip = 8, na.strings = "-32767")[,1:2]
...
sst_PI <- read.csv(here("data","SST_PI_2002_2019.csv"), skip = 8, na.strings = "-32767")[,1:2]
...
sst_amph <- read.csv(here("data",grep("Amphitrite", list.files(here("data")), value = TRUE)),
...
sst_race <- read.csv(here("data",grep("Race_Rocks", list.files(here("data")), value = TRUE)),
                     skip = 1, na.strings = "99.99")
...
# Wide format: rows are years, columns are months
cui <- read.csv(here("data","CoastalUpwellingIndex.csv"))[,-1]
colnames(cui)[1] <- "year"
...
biol_trans <- read.csv(here("data","biological_spring_transition_NWFSC.csv"),
